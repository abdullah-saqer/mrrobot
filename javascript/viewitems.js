...
///add new filter
    $('#filters*').on('click', '.list-group-item', function (e) {
        ...
        if (appliedFilters($(this).attr('data'), $(this).parent().attr('id'), 'add') == 1) {
            $('#applied-filters ul').append($('<li>').attr({
                'id': id,
                'data': filterType
            }).html($alias + '<i class="fa fa-times-circle-o" aria-hidden="true"onclick="removeFilter(this) "></i>'));
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            for (var i = 0; i < result.length; i++) {
                $('#category-content*').append($('<a>').attr({
                    'data-alias': result[i].name,
                    data: result[i].id,
                    class: 'list-group-item'
                }).html(result[i].name + '<span class="badge"></span>'));
            }
...
function printItem(data) {
    ...
    item += '<button type="button" id="add_to_cart"><i class="fa fa-shopping-cart"></i> <span class="hidden-xs hidden-sm hidden-md">Add to Cart</span></button>';
...
function getImage(photos) {
    ...
    $.each(photos, function (key, value) {
        if (value['primary'] == true) {
            path = "../Control-panel/items_photos/" + value['path'];
        }
